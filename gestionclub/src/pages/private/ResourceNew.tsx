...
export default function ResourceNew() {
  ...
  const [form, setForm] = useState({ name: "", type: "", campus: "", totalQuantity: 1 });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(clubId ? `/club/${clubId}` : "/resources");
    } catch (err: any) {
      setErr(err?.response?.data?.message || "Error al crear el recurso.");
    } finally {
      setLoading(false);
    }
  };
